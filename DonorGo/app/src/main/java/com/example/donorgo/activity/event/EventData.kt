...
object EventData {
    ...
    private val eventStatus = arrayOf(
        "On Going",
    ...
    private val eventLocation= arrayOf(
    ...
    val listData: ArrayList<Event>
        get() {
            ...
            for (position in eventTitle.indices) {
                val event = Event()
                event.detail = eventDetail[position]
                event.title = eventTitle[position]
                event.status = eventStatus[position]
                event.location = eventLocation[position]
